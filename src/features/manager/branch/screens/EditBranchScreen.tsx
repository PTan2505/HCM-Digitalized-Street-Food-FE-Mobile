...
} from '@manager/branch/hooks/useManagerBranch';
...
export const EditBranchScreen = (): JSX.Element => {
  const { t } = useTranslation();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const navigation = useNavigation<any>();
  const { data: branch } = useManagerBranchDetail();
  const updateBranch = useUpdateManagerBranch();
  const sessionIdRef = useRef(`branch-edit-self-${Date.now()}`);
  ...
  const onSubmit = (values: EditBranchFormValues): void => {
    ...
    updateBranch.mutate(
      ...
      },
      {
        ...
        onError: (error) => {
          console.error('Manager update branch failed:', error);
