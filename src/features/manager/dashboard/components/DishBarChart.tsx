...
export const DishBarChart = ({ data }: Props): React.JSX.Element => {
  ...
  const chartData = useMemo(() => {
    ...
    const collapsed =
      sorted.length > 5
        ? [
            ...
            {
              dishId: -1,
              dishName: t('manager_dashboard.others'),
              totalQuantityOrdered: sorted
                .slice(5)
                .reduce((acc, c) => acc + c.totalQuantityOrdered, 0),
            },
          ]
        : sorted;
    ...
    return collapsed.map((dish, idx) => ({
      value: dish.totalQuantityOrdered,
      label: truncate(dish.dishName, maxLen),
    ...
    <View className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      <View className="mb-3">
        <Text className="text-base font-bold text-gray-900">
          {t('manager_dashboard.top_dishes_chart_title')}
        </Text>
        <Text className="text-xs text-gray-500">
          {t('manager_dashboard.top_dishes_chart_subtitle')}
        </Text>
      </View>
      {chartData.length === 0 ? (
        <View className="h-48 items-center justify-center">
          ...
          <Text className="text-sm text-gray-400">
            {t('manager_dashboard.no_dish_data')}
          </Text>
        </View>
      ) : (
        <BarChart
